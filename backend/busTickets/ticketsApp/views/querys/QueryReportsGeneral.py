...
"""
select v.internal_number, t.generate_date, t.departure_time, concat(u.name, ' ', u.last_name), r.destiny, t.total_value,
...
where b.nit = '891800044-1' and t.departure_time between to_date('2023-01-12','yyyy-MM-dd') and to_date('2023-01-14','yyyy-MM-dd')
...
where t.generate_date between to_date('2023-01-12','yyyy-MM-dd') and to_date('2023-01-14','yyyy-MM-dd')
    and t.total_value NOT IN ('0') and b.nit = '891800044-1'
...
join "ticketsApp_client" c on t.client_id = c.id join "ticketsApp_user" u on c.user_id = u.id
join "ticketsApp_road" r on t.road_id = r.id join "ticketsApp_business" b on v.business_id = b.nit
where c.id = 6 and t.generate_date BETWEEN to_date('2023-01-12','yyyy-MM-dd') AND to_date('2023-01-14','yyyy-MM-dd')
AND t.total_value <> '0' and b.nit = '891800044-1'
...
from "ticketsApp_ticket" t
    ...
    where u.identification = '1244543343' and t.generate_date between to_date('2023-01-10','yyyy-MM-dd') and to_date('2023-01-12','yyyy-MM-dd')
    AND t.total_value <> '0'
...
"""
select v.internal_number, t.generate_date,t.departure_time, concat(u.name,' ',u.last_name), r.destiny,t.total_value, SUM(t.quantity) as quantity
from "ticketsApp_ticket" t join "ticketsApp_vehicle" v on t.vehicle_id = v.plate
join "ticketsApp_boxoffice" bo on t.box_office_id = bo.id join "ticketsApp_user" u on bo.user_id = u.id
join "ticketsApp_road" r on t.road_id = r.id join "ticketsApp_business" b on v.business_id = b.nit
where t.total_value not in ('0') and t.generate_date between to_date('2023-01-12','yyyy-MM-dd') and to_date('2023-01-14','yyyy-MM-dd')
and b.nit = '891800044-1' and bo.id = 3
...
Reporte despachos por usuario (Descuentos por usuario inge) TABLA Planilla y box_office y vehicle
"""
